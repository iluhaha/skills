...
**КРИТИЧЕСКОЕ ПРАВИЛО: Стратегически важные данные — 90% достоверность или запрос у человека.**

Данные, на которых строятся цели и KPI стратегии (посещаемость, выручка, доля рынка, размер аудитории, конверсия), ДОЛЖНЫ быть верны с вероятностью ≥90%. Для этого:

1. **Найти в источниках** — искать в P1-P4 источниках, официальных отчётах, пресс-релизах компании
2. **Если не найдено** — НЕ считать по косвенным данным (пропускная способность ≠ реальная посещаемость). Вместо этого **запросить у человека**: "Для точной стратегии мне нужна реальная посещаемость [бренда]. Вы можете предоставить эту цифру?"
3. **Если эстимация неизбежна** — чётко отделить от фактов:
   - Пометить `[⚠ эстимация — требуется уточнение у клиента]`
   - Указать метод расчёта и его ограничения
   - НЕ строить цели/KPI на эстимациях — зафиксировать как "цель будет скорректирована после получения реальных данных"
4. **Никогда не выдавать расчётный потолок за реальный факт** — максимальная пропускная способность, ёмкость зала, количество мест — это НЕ реальная загрузка

Список данных, которые ВСЕГДА требуют ≥90% достоверности:
- Посещаемость / трафик бренда
- Выручка / продажи бренда и конкурентов
- Размер целевой аудитории
- Конверсия по воронке
- Текущие маркетинговые бюджеты
- Цены бренда и конкурентов
</HARD-GATE>
